...
public class StringUtilities {
	...
	public static String rewriteUrlForCdn(String url, String cdnHost) {
		if (StringUtilities.isEmpty(cdnHost)) {
			return url;
		}
		...
		StringBuffer result = new StringBuffer(firstParts[0] + "://" + cdnHost);
		for (int index = 1; index < secondParts.length; index++) {
			result.append("/" + secondParts[index]);
		}
		
		//add the fallback URL for this resource
		char delimiter = result.indexOf("?") < 0 ? '?' : '&';
		try {
			result.append(delimiter + "fallback=" + URIUtil.encodeWithinQuery(url));
		}
		catch (Exception e) {
			//LOG.warn("Failed to include callback URL; url=" + url + ", rewritten=" + result.toString());
		}
